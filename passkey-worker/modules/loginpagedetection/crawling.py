...
from common.modules.locators.anchor import AnchorLocator
...
class Crawling:
    ...
    def __init__(self, config: dict, result: dict):
        ...
        self.login_page_element_keywords = config.get("login_page_config", {}).get("login_page_element_keywords", ["login", "signin"])
        self.max_anchor_candidates = config.get("login_page_config", {}).get("crawling_strategy_config", {}).get("max_anchor_candidates", 1)
        self.max_element_candidates = config.get("login_page_config", {}).get("crawling_strategy_config", {}).get("max_element_candidates", 1)
        self.max_elements_to_click = config.get("login_page_config", {}).get("crawling_strategy_config", {}).get("max_elements_to_click", 10)
