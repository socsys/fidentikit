...
class Searxng:
    ...
    def __init__(self, config: dict, result: dict):
        ...
        self.search_engines = config.get("login_page_config", {}).get("metasearch_strategy_config", {}).get("search_engines", [])
        self.search_term = config.get("login_page_config", {}).get("metasearch_strategy_config", {}).get("search_term", "%s login")
        self.search_results_number = config.get("login_page_config", {}).get("metasearch_strategy_config", {}).get("search_results_number", 10)
