...
class SitemapFetcher(object):
    ...
    __slots__ = [
        ...
        '_recursion_level',
        '_web_client',
        '_max_recursion_level',
        '_max_sitemap_size'
    ]

    def __init__(self, url: str, recursion_level: int, web_client: Optional[AbstractWebClient] = None,
                 max_recursion_level: int = 10, max_sitemap_size: int = 50 * 1024 * 1024):

        if recursion_level > max_recursion_level:
            log.info(f"Recursion level ({recursion_level}) exceeded max limit ({max_recursion_level}) for: {url}")
            raise SitemapException("Recursion level exceeded {} for URL {}.".format(max_recursion_level, url))
        ...
        self._max_sitemap_size = max_sitemap_size
    ...
    def sitemap(self) -> AbstractSitemap:
        ...
        if response_content[:20].strip().startswith('<'):
            ...
            parser = XMLSitemapParser(
                ...
                max_sitemap_size=self._max_sitemap_size
            )
        ...
        log.info("Parsing level {} sitemap from: {}".format(self._recursion_level, self._url))
...
class AbstractSitemapParser(object, metaclass=abc.ABCMeta):
    ...
    def __init__(self, url: str, content: str, recursion_level: int, web_client: AbstractWebClient,
                 max_recursion_level: int, max_sitemap_size: int):
        self._url = url
...
class IndexRobotsTxtSitemapParser(AbstractSitemapParser):
    ...
    def __init__(self, url: str, content: str, recursion_level: int, web_client: AbstractWebClient,
        ...
        super().__init__(url=url, content=content, recursion_level=recursion_level, web_client=web_client,
                         max_recursion_level=max_recursion_level, max_sitemap_size=max_sitemap_size)
...
class XMLSitemapParser(AbstractSitemapParser):
    ...
    def sitemap(self) -> AbstractSitemap:
        ...
        try:
            ...
            log.info("Failed to parse sitemap: {}".format(ex))
