...
from common.lib.usp.tree import sitemap_tree_for_homepage
from common.lib.usp.web_client.requests_client import RequestsWebClient
from common.lib.usp.exceptions import SitemapException, \
...
class Sitemap:
    ...
    def __init__(self, config: dict, result: dict):
        ...
        sitemap_strategy_config = login_page_config.get("sitemap_strategy_config", {})
        artifacts_config = config.get("artifacts_config", {})
        self.login_page_url_regexes = login_page_config.get("login_page_url_regexes", [])
        self.max_candidates = sitemap_strategy_config.get("max_candidates", 10)
        self.max_recursion_level = sitemap_strategy_config.get("max_recursion_level", 2)
        self.max_sitemap_size = sitemap_strategy_config.get("max_sitemap_size", 50000)
        self.timeout_fetch_sitemap = sitemap_strategy_config.get("timeout_fetch_sitemap", 10)
        self.store_sitemap = artifacts_config.get("store_sitemap", False)
